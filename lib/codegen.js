...
const generateOperationFile = (config, operation, operation_name) => new Promise((resolve, reject) => {
  ...
  fs.readFile(path.join(config.root, config.file_name), 'utf8', (err, data) => {
    ...
    // console.log ('requestBody', operation && operation[0].path && operation[0].path.post && operation[0].path.post.requestBody && JSON.stringify(operation[0].path.post.requestBody));
    // console.log ('query', operation && operation[0].path && operation[0].path.get && operation[0].path.get.parameters);
    // console.log ('headOperation', headOperation);
